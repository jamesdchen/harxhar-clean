...
Reads results/vrp_stage2b_spec.json: {"survivors": [col, ...], "out": "results/vrp_stage2"}.
...
SPEC = "results/vrp_stage2b_spec.json"


def main() -> None:
    spec = json.load(open(SPEC))
    surv: list[str] = spec["survivors"]
    out = spec.get("out", "results/vrp_stage2b")
    ...
    p = fit_predict_ridge(
        X, y, train_win_periods=144_000, hyperparams=dict(alpha=1.0, _refit_frequency=1)
    )

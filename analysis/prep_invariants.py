...
def audit(cache_dir, label):
    ...
    for c in sorted(labels):
        ...
        j1 = names.index(f"adj_{c}_ma_1") if f"adj_{c}_ma_1" in names else None
        if j1 is not None:
            col = np.asarray(X[:, j1], np.float64)
            mn, md = rolling_iqr_stats(col)
            rec["iqr_min"], rec["iqr_median"] = mn, md
            ratio = mn / md if (md and np.isfinite(md) and md > 0) else np.nan
            rec["iqr_ratio"] = ratio
            if np.isfinite(ratio) and ratio < IQR_COLLAPSE_RATIO:
                fails.append(f"[B] {c}: rolling IQR collapses to "
                             f"{ratio:.4f} of its median")
...
def main():
    ...
    out = {}
    for t in targets:
        if not os.path.exists(os.path.join(t, "X.npy")):
            print(f"skipping {t}: not built")
            continue
        lbl = os.path.basename(t)
        out[lbl] = audit(t, lbl)
    if len(out) > 1:
        ks = list(out)
        print(f"\n{'=' * 86}")

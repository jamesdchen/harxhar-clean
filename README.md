# harxhar — repository guide

## Branch layout

| Branch | Role |
|---|---|
| `master` | Campaign code and results: executors, SGE jobs, experiments, harvested scores. Day-to-day working branch. |
| `main` | The paper: `writeup/` sources, generated macros, PDF. Paper-only commits plus mechanical "port master" notes. |
| `paper2` | Mirror of `main` maintained by the port process. |
| `archive/*` (tags) | Frozen heads of retired feature branches (see `git tag -l 'archive/*'`). |

## Current state (2026-08)

- Scoring protocol: calibrated back-transformation with a causal EWMA second
  moment (lambda = 0.6); every scored arm inherits it. Legacy scalar-protocol
  arms are labeled in the paper's appendix.
- Options ingestion: `src/data/options_features.py` is the single door for
  options data (go-live publication lag + staleness cutoff enforced in code);
  channels register as the `options` bucket in `src/data/loading.py`,
  deliberately outside `ALL_FEATURES` until a panel-version bump.
- Cluster jobs are single-slot by policy: parallel-environment requests
  throttle the queue; threads follow `$NSLOTS`.

...
moe () {  # gdepth gtemp label
  local J C
  J=$(qsub -terse -v CID=$CID,GDEPTH=$1,RDEPTH=2,GATEKIND=tree,NREG=4,$E,GTEMP=$2,RDIAG=false,MOE_DIAG=1,LBL=$3 moe_dig.sge | cut -d. -f1)
  C=$(qsub -terse -hold_jid "$J" -v CID=$CID,ARM=resid_regime,LBL=$3 sig_collect.sge)
  echo "  $3 (gdepth=$1 gtemp=$2) dig=$J col=$C"
}
noreg () {  # gdepth label
  local J C
  J=$(qsub -terse -v CID=$CID,GDEPTH=$1,NOREG=1,LBL=$2 moe_dig.sge | cut -d. -f1)

...
BDIG=$(sbatch --parsable --export=ALL,CID=$BCID,ARM=resid_regime,LBL=$LBL sig_dig.sbatch)
BCOL=$(sbatch --parsable --dependency=afterany:$BDIG --export=ALL,CID=$BCID,ARM=resid_regime,LBL=$LBL sig_collect.sbatch)
...
for W in "15 19" "17 19" "16 20" "16 18"; do
  ...
  P=$(sbatch --parsable --export=$E sweep_prep.sbatch)
  M=$(sbatch --parsable --dependency=afterok:$P --export=$E,CID=$CID sig_masks.sbatch)
  D=$(sbatch --parsable --dependency=afterok:$M --export=$E,CID=$CID,ARM=resid_regime,LBL=$LBL sig_dig.sbatch)
  C=$(sbatch --parsable --dependency=afterany:$D --export=$E,CID=$CID,ARM=resid_regime,LBL=$LBL sig_collect.sbatch)

...
the RV–IV notebook §14). Short iron condor = short body + long wings.
...
Condor sizes follow the RV–IV §14 convention (unit-median leverage from
...
§14 of the RV–IV notebook prices the condor around the straddle body.
...
further OTM ($w=25,50$; the RV–IV notebook's §14 construction) and
$R_{\mathrm{long,ic}}=(\mathrm{exit}_{ic}-\mathrm{entry}_{ic})/\mathrm{width}$,
...
on the common days), so it can differ slightly from the RV–IV §14 row,
...
# condor w=25, §14 conventions; blk2 straddle-book UM sizes on both legs
body = atm.reset_index()
close_map = pd.Series(atm["S_close"].to_numpy(), index=pd.to_datetime(atm["expiration"]).values)
close_map.index = pd.to_datetime(close_map.index).tz_localize(None).normalize()

...
"""RFF correction: PCovR projection + 256 random Fourier features
read by a PER-BAR rolling ridge (fresh read-out; the kNN is segment-stale): each segment harvests its own top-24 cells
...
for seg in range(0, n1 - n0, 240):
    ...
    Fb = np.hstack([Kc[lo:hi, None] for Kc in cores] + [C, X[lo:hi][:, cal_cols]])
    ...
    Z = ((V - mc._mean) / mc._std) @ mc._R
    rng = np.random.default_rng(1000 + seg)
    sub = Z[rng.choice(len(Z), 3000, replace=False)]
    pa, pb_ = rng.integers(0, 3000, 6000), rng.integers(0, 3000, 6000)
    sigma = np.median(np.linalg.norm(sub[pa] - sub[pb_], axis=1))
    DR = 256
    Om = rng.normal(0.0, 1.0 / sigma, (Z.shape[1], DR))
    ph = rng.uniform(0.0, 2 * np.pi, DR)
    PHI = np.sqrt(2.0 / DR) * np.cos(Z @ Om + ph)
    rr_c = RollingRidgeResidualizer(1.0, fast_inverse=True)
    rr_c.fit(PHI, resid[W:])
    phi_hist = list(PHI[-1:])  # rolling buffers
    resid_hist = list(resid[W:])
    ...
    for i in range(hi - t):
        ...
        zq = ((q - mc._mean) / mc._std) @ mc._R
        phq = np.sqrt(2.0 / DR) * np.cos(zq @ Om + ph)
        corr = float(rr_c.predict(phq[None, :])[0])
        if len(stk_c) >= 1000:
        ...
        new_resid = float(y[t + i]) - base
        resid_roll.append(new_resid)
        # per-bar freshness: roll the RFF read-out with the realized residual
        rr_c.roll(phq, new_resid, PHI[i], resid[W + i])
        if i < hi - t - 1:

...
n = X.shape[0]
raw = load_raw_data("data", allow_missing=True).dropna(subset=["RV"]).reset_index(drop=True)
...
BINS = [("overnight", 20.0, 28.0), ("early", 4.0, 9.5), ("open", 9.5, 11.0),
        ("midday", 11.0, 14.5), ("close", 14.5, 16.0), ("after", 16.0, 20.0)]
def binmask(lo, hi):
    h = hours % 24
    if hi <= 24: return (h >= lo) & (h < hi)
    return (h >= lo) | (h < hi - 24)

def probe(mask):
    idx = np.flatnonzero(mask)
    p = 1 + len(other_cols) + len(ladder_cols)
    G = np.eye(p); G[0, 0] = 0.0
    c = np.zeros(p)
...
print(f"{'bin':<10} {'n':>7} | target: ||k||, cos(k, k_all) | exog shape amplitudes a1..a5")
for nm_, lo, hi in BINS:
    ...
    print(f"{nm_:<10} {nb:>7} | {np.linalg.norm(ktar_b):6.3f}  {cos:+.3f}  span={covg:.2f} | "

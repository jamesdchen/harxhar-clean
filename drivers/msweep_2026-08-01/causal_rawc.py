...
"""CAUSAL cell re-selection: each segment harvests its own top-24 cells
...
for seg in range(0, n1 - n0, 240):
    ...
    Wrot = np.asarray(mc._R)
    if Wrot.shape[0] != V.shape[1]:
        Wrot = Wrot.T
    agg = np.abs(Wrot).sum(axis=1)[W:]
    ranked = np.argsort(agg)[::-1]
    sel = []
    for i in ranked:
        if state_cols[i] in non_cal_state:
            sel.append(state_cols[i])
        if len(sel) == NSEL:
            break
    F = np.hstack([Fb, X[lo:hi][:, sel]])
    ...
    print(
        f"[{TAG}] seg {seg // 240 + 1}/10 top: {[names[s] for s in sel[:4]]} ({time.time() - t0:.0f}s)",
        flush=True,
    )

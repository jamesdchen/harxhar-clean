...
O = X[:, other_cols]
cal_cols = [names.index(nm) for nm in
            ["DOW_0", "DOW_1", "DOW_2", "DOW_3", "DOW_4", "hour", "is_overnight", "is_open", "is_close"]]
bar1 = {c: X[:, chan[c][0][1]] for c in labels}

def ridge_fit(A, yy):
    G = A.T @ A + np.eye(A.shape[1]); G[0, 0] -= 1.0
...
DROP = {"stocktwits_attention", "stocktwits_sentiment", "stocktwits_sentcount",
        "vvix", "vix3m", "voldemand_spx_open_and_close", "voldemand_spx_open_only",
        "voldemand_all_open_and_close", "voldemand_all_open_only"}
...
RAWCELLS = ["adj_sumautocov_ma_128", "adj_sumvolume_ma_8", "adj_numobs_ma_16",
            "adj_spread_vwstock_ma_8", "adj_sumautocov_ma_4", "adj_sumvolume_ma_4",
            "adj_sumautocov_ma_1", "adj_sumpret2_ewstock_ma_64", "adj_sumautocov_ma_2",
            "adj_sumret4_ma_64", "adj_sumret4_ma_1", "adj_sumret3_ewstock_ma_32",
            "adj_sumautocov_ma_64", "adj_sumret_ma_2", "adj_sumpret2_vwstock_ma_128",
            "adj_sumret4_ma_32", "adj_sumabsret_ma_4", "adj_sumbipow_vwstock_ma_32",
            "adj_spread_ewstock_ma_16", "adj_sumautocov_ma_16", "adj_sumret_ma_64",
            "adj_sumabsret_ma_2", "adj_spread_vwstock_ma_2", "adj_spread_vwstock_ma_64"]
if os.environ.get("PB_CELLS"):
    _keep = os.environ["PB_CELLS"].split(",")
    RAWCELLS = [c for c in RAWCELLS if any(s in c for s in _keep)]

def run(tag, alpha=1.0, K=5, mixture=False, cadence=240, eff=10.0, pool_mode="kbar",
        products=False, prune=None, texture=0.0):
    ...
    P = np.empty(n1 - n0); Kbar = None; Pbar = None
    lam = 1.0 - 1.0 / eff
    t0 = time.time()
    if mixture:
        KM_T = np.stack([kfeat_series(y, be) for be in MIX_T], axis=1)
        KM_C = np.stack([kfeat_series(bar1[c], be) for c in labels for be in MIX_C], axis=1)
    for seg in range(0, n1 - n0, cadence):
        ...
        if mixture:
            ...
            har_idx = [oi for oi, j in enumerate(other_cols)
            ...
            F = np.hstack([TB] + [Kc[lo:hi, None] for Kc in cores2]
                          + [C, X[lo:hi][:, cal_cols]])
        else:
            ...
            if products:  # channel-core curvature (the prod_sq winner)
                F = np.hstack([F] + [np.column_stack([Kc[lo:hi] ** 2 for Kc in cores[1:]])])
            ...
            if cmod:
                ...
                if cmod == "fourier":
                    ...
                    mods = [X[lo:hi][:, names.index(g)] for g in
                            ("is_open", "is_close", "is_overnight")]
            ...
            for i in range(hi - t):
                ...
                if len(run.pnl_y) >= 1000:
                    if _os.environ.get("PB_PANEL") == "hedge":
                        ...
                        wts = (1.0 / mse); wts /= wts.sum()
                        P[seg + i] = float(wts @ np.array(preds))
                    else:
                        A2 = np.column_stack([np.ones(1000)] + [np.array([p[k] for p in run.pnl_p[-1000:]]) for k in range(3)])
                        wf, *_ = np.linalg.lstsq(A2, np.array(run.pnl_y[-1000:]), rcond=None)
                        P[seg + i] = wf[0] + sum(wf[1 + k] * preds[k] for k in range(3))
                else:
                    P[seg + i] = preds[1]
                run.pnl_p.append(preds); run.pnl_y.append(float(y[t + i]))
                if i < hi - t - 1:
                    for r_ in rrs:
    ...
    if START == 24000:
        ...
        print(f"[pb] {tag:<10} qlike={mm['qlike']:.6f} mz={mm['mz_beta']:.3f} p={F.shape[1]+1} "
              f"| vs hybrid_dp diff={r['mean_diff']:+.5f} p={r['p']:.4f} ({time.time()-t0:.0f}s)", flush=True)
    else:
        print(f"[pb] {tag:<10} qlike={mm['qlike']:.6f} mz={mm['mz_beta']:.3f} p={F.shape[1]+1} "
              f"| tile [{START},{END}) ({time.time()-t0:.0f}s)", flush=True)

...
Prints the cost estimate first (cents for ES.v.0 ohlcv-1m over two years) and
aborts above --max-usd.  The continuous volume-ranked front month ``ES.v.0``
...
def main(argv: list[str] | None = None) -> int:
    ...
    df = store.to_df(pretty_ts=True, pretty_px=True, map_symbols=True).reset_index()
    out = Path(a.out)

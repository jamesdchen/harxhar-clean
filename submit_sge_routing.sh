...
sub () {  # depth gatekind nreg rfull label
  local J C
  J=$(qsub -terse -v CID=$CID,RDEPTH=$1,GATEKIND=$2,NREG=$3,RFULL=$4,EXPERT=fm,BASIS=hinge,NKNOTS=3,RANK=4,FMLIN=false,WDV=1.0,WD=0.1,RDIAG=false,LBL=$5 moe_dig.sge | cut -d. -f1)

...
sub () {  # cid rfull glr label
  local J C
  J=$(qsub -terse -v CID=$1,RDEPTH=2,GATEKIND=film,GCTX=raw,NREG=4,$E,RFULL=$2,GLR=$3,RDIAG=false,MOE_DIAG=1,LBL=$4 jobs/sge/moe_dig.sge | cut -d. -f1)
  C=$(qsub -terse -hold_jid "$J" -v CID=$1,ARM=resid_regime,LBL=$4 jobs/sge/sig_collect.sge)

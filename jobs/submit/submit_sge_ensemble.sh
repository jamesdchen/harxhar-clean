...
sub () {  # ens_w mt_auxw label
  local J C
  J=$(qsub -terse -v CID=$CID,RMODEL=ebm_mtfm,RDEPTH=2,EXPERT=fm,BASIS=hinge,NKNOTS=3,ENS_W=$1,MT_AUXW=$2,MT_NBAGS=8,MT_RANK=4,MT_EPOCHS=250,EBM_CFG={},RDIAG=false,LBL=$3 jobs/sge/moe_dig.sge | cut -d. -f1)

...
JID=$(sbatch --parsable --export=ALL,CID=$CID,BASE=$BASE jobs/slurm/carc_ggrid.sbatch)
...
for k in auniform_w20 auniform_w30 auniform_w40 aghat_w20 aghat_w30 aghat_w40; do
  C=$(sbatch --parsable --dependency=afterany:$JID --export=ALL,CID=$CID,ARM=resid_regime,LBL=${BASE}_${k} jobs/slurm/carc_collect.sbatch)

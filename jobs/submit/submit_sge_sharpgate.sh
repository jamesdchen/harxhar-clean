...
sub () {  # gtemp label
  local J C
  J=$(qsub -terse -v CID=$CID,RDEPTH=2,GATEKIND=tree,NREG=4,EXPERT=fm,BASIS=hinge,NKNOTS=3,RANK=4,FMLIN=false,WDV=1.0,WD=0.1,GWD=0.0,AUXW=0.0,GTEMP=$1,RDIAG=false,MOE_DIAG=1,LBL=$2 jobs/sge/moe_dig.sge | cut -d. -f1)

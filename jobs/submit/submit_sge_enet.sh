...
moe () {  # rinv rnog gtemp label
  local J C
  J=$(qsub -terse -v CID=$CID,RDEPTH=2,GATEKIND=tree,NREG=4,$E,RFULL=1,RINV=$1,RNOG=$2,GTEMP=$3,RDIAG=false,MOE_DIAG=1,LBL=$4 jobs/sge/moe_dig.sge | cut -d. -f1)
...
echo ENET_SUBMITTED
J=$(qsub -terse -v CID=$CID,NOREG=1,LBL=en_noreg jobs/sge/moe_dig.sge | cut -d. -f1)
C=$(qsub -terse -hold_jid "$J" -v CID=$CID,ARM=resid_regime,LBL=en_noreg jobs/sge/sig_collect.sge)

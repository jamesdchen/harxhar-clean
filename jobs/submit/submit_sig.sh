...
LBL=sig
PREP=$(sbatch --parsable jobs/slurm/sig_prep.sbatch)
...
for CID in xgb_all_buckets_tw1000_enetreg2_sig_rf480_slim xgb_all_buckets_tw1000_enetreg2_sigrel_rf480_slim; do
  MASK=$(sbatch --parsable --dependency=afterok:$PREP --export=ALL,CID=$CID jobs/slurm/sig_masks.sbatch)
  echo "  $CID masks=$MASK"
  for ARM in resid_subset resid_regime; do
    DIG=$(sbatch --parsable --dependency=afterok:$MASK --export=ALL,CID=$CID,ARM=$ARM,LBL=$LBL jobs/slurm/sig_dig.sbatch)
    COL=$(sbatch --parsable --dependency=afterany:$DIG --export=ALL,CID=$CID,ARM=$ARM,LBL=$LBL jobs/slurm/sig_collect.sbatch)

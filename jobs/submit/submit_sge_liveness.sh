...
sub () {  # gwd auxw label
  local J C
  J=$(qsub -terse -v CID=$CID,RDEPTH=2,GATEKIND=tree,NREG=4,EXPERT=fm,BASIS=hinge,NKNOTS=3,RANK=4,FMLIN=false,WDV=1.0,WD=0.1,GWD=$1,AUXW=$2,RDIAG=false,MOE_DIAG=1,LBL=$3 jobs/sge/moe_dig.sge | cut -d. -f1)

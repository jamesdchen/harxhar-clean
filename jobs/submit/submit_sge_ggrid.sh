...
BASE=ggrid
J=$(qsub -terse -v CID=$CID,BASE=$BASE jobs/sge/moe_ggrid.sge | cut -d. -f1)
...
for k in auniform_w20 auniform_w30 auniform_w40 aghat_w20 aghat_w30 aghat_w40; do
  C=$(qsub -terse -hold_jid "$J" -v CID=$CID,ARM=resid_regime,LBL=${BASE}_${k} jobs/sge/sig_collect.sge)

...
| 12 oracle hold to close | **EDA, not a candidate**: the remaining-session oracle `sign(RV_rem − IV_rem)` — the session's realized variance from clock `t` to the close against the market's price of that same remainder — applied to the nearest-OTM straddle picked at `t` and HELD to cash settlement (one crossing at entry, no re-pick, no exit spread), described by clock on the deck's 866 days; 10,387 packages, 10,374 with a vendor implied. Distinct from 10's 30-minute slice oracle | oracle held to settlement 3.06–4.72 mid / 2.82–4.35 crossed by entry clock (15:30 4.54 / 4.10, 15:00 4.72 / 4.35); always short held −0.38 to +0.78 mid, −0.66 to +0.39 crossed | gates: BSM at the vendor hourly implied over the remaining window reproduces the package midpoint (median ratio 1.0000 at every clock); the 15:30 row reproduces the deck to 9.5e-07 (rule table sign(s) 1.3383, always short 0.2038); the payoff identity — terminal displacement from the strike centre less half the pair's width, floored at zero — is exact; the peek moves the sign on 67.1–71.1% of daytime rows and 0 of 861 at 15:30 | **A DESCRIPTION, NOTHING ADOPTED.** The remainder is rich at every clock and only mildly richer into the close (median RV_rem/IV_rem 0.835 at 10:00, 0.757 at 15:30; short on 68.1–75.2%). The sign never flips across the twelve clocks on 44.1% of days and is asymmetric: short→short 92.0% of consecutive pairs against long→long 79.6%, so a 10:00 short survives to 15:30 on 78.4% of days and a 10:00 long on 39.9%. The money is the short leg late (2.30 at 10:00 to 4.19 at 15:00, against a flat 1.9–2.7 long leg), and the crossing costs the ceiling only 6–12% of its Sharpe. The held straddle stops being a variance instrument in the MORNING: it pays the terminal displacement from strikes 3.70 strikes away at the median from 10:00 against 1.26 from 15:30, where the index lands carries 92–95% of its P&L variance at every clock, and it delivers 31% of the Sharpe a payoff proportional to the day's realized volatility would give from 10:00 against 137% from 15:30. Conditionals (descriptive): FOMC 84.6% short against 71.4% other; a morning that realized 0.67 of its own price is followed by an afternoon short on 78.8% of clocks against 66.2% after a morning that realized 1.54 times its price. Files 12_oracle_hold_to_close.{py,md}, results/atm_straddle_intraday/proposals/12/. |
| 13 terminal move | **pre-registered four-cell study of the payoff's actual argument**: the settled straddle pays on the squared terminal move r^2, not on RV, so T0 = the deck's sign(s); T1 = the same weighted MZ map re-targeted at y = sqrt(r^2/B), giving tv_hat; T2 = the expected payoff of the ACTUAL package under a normal terminal return with variance tv_hat against the midpoint premium (plus a bid/ask hurdle variant and an rv_hat secondary); T3 = an expanding prior-days-only regression of log r^2 on log tv_hat plus five 15:30 features (|r| 15:00-15:30, net return 10:00-15:30, today's RV so far relative to trailing, distance to the nearest 25-point strike over the implied move, FOMC/month-end) | T0 1.338 / 0.870; T1 0.525 / 0.047; T2 0.466 / -0.011; T2h 0.664 / 0.218; T2b 1.318 / 0.850; T2bh 1.385 / 0.962; T3 -0.491 / -0.789 (mid / crossed, 866 days) | gate: the deck's rule table reproduced to 4.2e-07 and rv_hat rebuilt from the panel to exactly 0; causality 0/10 on tv_hat, on the T3 forecast and on the T3 position; RV/r^2 = 1.047 pooled over 68,215 session bars but 0.765 on the 16:00 stamp and 0.676 on the traded bars; QLIKE against r^2 1.4869 (tv_hat) against 1.4350 (rv_hat), DM t +2.92, and rv_hat wins on all eight forecasts (DM t +1.73 to +2.99); every dSharpe interval covers zero except T3's, which excludes zero AGAINST T3; the one cell that beats T0 crossed is T2bh (+0.092, [-0.359, +0.514], placebo 99.6th) and it uses T0's own forecast | **REJECT all four: the terminal move is the right target and the recalibration is the wrong instrument.** The premise holds for the day (RV exceeds r^2 by 4.7% pooled) and REVERSES at the bar that pays (0.676), so the RV map is biased low, not high, and re-targeting makes it worse - one squared draw is a far noisier regressand than thirty summed squares, and the pooled twelve-clock fit lands tv_hat 16% below rv_hat exactly where it needed to be higher. T3 degenerates to always long because exp(s2/2) = 10.29 is the wrong retransformation for the log of a squared normal (the smearing factor is 4.01); the level-corrected version is post-hoc, still worse-calibrated than T0, and not adopted. What survives is the measurement: the terminal-move oracle sign(r^2 - iv_var) is 0.73 correlated with the sign of the trade's profit and right on 87% of days, against the variance oracle's 0.17 and 63% (they agree on 69.2%) - the information is there and this instrument does not reach it. Files 13_terminal_move.{py,md}, results/atm_straddle_intraday/proposals/13/. |

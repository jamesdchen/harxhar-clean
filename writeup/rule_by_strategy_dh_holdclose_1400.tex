% Standalone render of generated_intraday_dh/table_rule_by_strategy_dh_holdclose_1400.tex
% Build: pdflatex -interaction=nonstopmode rule_by_strategy_dh_holdclose_1400.tex
...
\begin{document}
\begin{center}
\textbf{0DTE nearest-OTM straddle: per-rule return summary, entry 14:00 ET, held to cash-settle, models compared on the same 866 days}\\[1.5ex]
\input{generated_intraday_dh/table_rule_by_strategy_dh_holdclose_1400}
\end{center}
\vspace{1ex}
\begin{center}
\includegraphics[width=0.45\textwidth]{../results/atm_straddle_intraday_holdclose/rule_by_strategy_dh/1400/rule_hists_blk2_1400.png}\par\smallskip
...
\end{center}
\vspace{1ex}
\noindent\small One trade a day: enter the nearest-OTM straddle at 14:00 ET, hold those strikes to the official close, and delta-hedge every 30 minutes. 866 expiration days, 2020-01-03 to 2024-04-30 (the deck's frame). Fills are at the quoted midpoint everywhere except the last column, which is the same rule at the crossed spread: entry at the touch (the ask when long, the bid when short) and cash-settle at the official close (no exit spread).
\vspace{1ex}

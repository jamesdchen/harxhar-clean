...
We consider five models spanning parametric linear, ensemble tree-based, and naive approaches. All models are trained on the preprocessed (diurnally adjusted, transformed, winsorized) scale. Every arm is refit at every bar. For the linear arms this is made tractable by the exact rank-1 and homotopy updates of Section~\ref{sec:exact_estimators}; for the tree arms it is a deliberate expense, incurred so that refit cadence cannot confound the model-class comparison of Section~\ref{sec:linvsnonlin}.
...
\begin{equation}
    ...
    \label{eq:ridge}
\end{equation}
with regularization parameter $\alpha$ re-selected causally (Section~\ref{sec:tuning}); the shared incumbent is the $\alpha = 0$ limit of this path, i.e.\ exact per-bar OLS. Input features are standardized using a rolling robust scaler based on the median and interquartile range (IQR) estimated from the current training buffer. Ridge is refit at every time step via the closed-form solution.

\subsubsection{XGBoost}
\label{sec:xgboost}

We employ XGBoost \citep{Chen2016} as a gradient-boosted tree ensemble with the histogram-based splitting method. Hyperparameters are re-selected causally by the protocol of Section~\ref{sec:tuning}; the learning rate is pinned at the production value of 0.1 and the search ranges over max-depth and column subsampling. The model is refit at every bar.

\subsubsection{LightGBM}
\label{sec:lightgbm}

LightGBM \citep{Ke2017} is estimated under the same causal selection protocol and search grid as XGBoost, and is likewise refit at every bar.

\subsubsection{Random Forest}
\label{sec:rf}

A random forest ensemble is estimated under the same causal selection protocol. It does not appear in the frozen battery of Sections~\ref{sec:marginal}--\ref{sec:algorithms} and is retained here only for completeness of the model description.

\subsubsection{Hyperparameter Selection}
\label{sec:tuning}

All hyperparameters reported in this paper are selected by the \emph{causal}
rolling protocol of Section~\ref{sec:causal_tuning}: every 250 solves, each arm
re-selects its configuration on a leakage-clean forward split of its own
current training window (fit block, 25-bar embargo covering the longest moving
average, 125-bar validation tail scored by mean squared error). No forecast's
hyperparameters are selected using information from that forecast's future, and
the selection loss is never the evaluation loss.

\paragraph{A superseded protocol, retained as a cautionary result.}
An earlier version of this study instead performed Bayesian optimization with
Optuna \citep{Akiba2019} whose objective was QLIKE \emph{on the full
walk-forward backtest} --- that is, on the evaluation panel itself, under the
evaluation loss. Trial counts also differed sharply across models, so arms were
ranked partly by how many draws each had taken from the evaluation set. We
report the magnitude of the resulting distortion in
Section~\ref{sec:tuning_leak} because it calibrates how large a protocol
artifact can be on this task; no number produced under that protocol appears
...
All models are evaluated using a strictly out-of-sample walk-forward procedure. At each test period $t$, the model is trained (or updated) on a rolling window of $T_{\mathrm{train}}$ consecutive observations immediately preceding $t$. \textbf{Every result reported in this paper uses $T_{\mathrm{train}} = 24{,}000$ bars} (approximately 500 trading days at 48 bars per 24-hour day), with model refit at every bar for every arm, so that no refit-cadence asymmetry confounds the model-class comparison. A shorter $T_{\mathrm{train}} = 500$ window and a five-step tree refit cadence were used in superseded experiments; those settings produce none of the numbers reported here and are noted only because earlier internal tables used them. The procedure is:
...
\begin{equation}
    ...
    \label{eq:duan}
\end{equation}
where $\hat{\sigma}_{\varepsilon}^2 = \frac{1}{N}\sum_{i}(y_i - \hat{y}_i)^2$ is
the in-sample mean squared residual and $\bar{RV}_{t \mid s}^{\mathrm{baseline}}$
is the diurnal baseline factor at time $t$ that reverses Stage 1.
Equation~\eqref{eq:duan} is the analytic form of this correction rather than
Duan's empirical smearing average $\frac{1}{N}\sum_i (\hat{y}_t + \hat\varepsilon_i)^2$;
the two coincide when the residuals are mean-zero, since
$\mathbb{E}[(\hat y + \varepsilon)^2] = \hat y^2 + 2\hat y\,\mathbb{E}[\varepsilon] + \sigma^2_\varepsilon$.
Two biases are left uncorrected and we do not quantify them: $\hat\sigma^2_\varepsilon$
is estimated on the winsorized scale while the evaluation target is unwinsorized,
and Stage 3 is not inverted at all (Section~\ref{sec:limitations}).
...
\begin{equation}
    ...
    \label{eq:qlike}
\end{equation}
QLIKE is a strictly consistent scoring rule for the conditional variance, and
its practical appeal in this literature is robustness: \citet{Patton2011} shows
that the ranking it induces is preserved when the target is a noisy but
conditionally unbiased proxy for integrated variance, which a realized measure
is. It penalizes under-prediction more heavily than over-prediction. That
asymmetry is not neutral for model comparison --- Section~\ref{sec:calibration}
shows that on this panel QLIKE systematically prefers attenuated forecasts ---
and Section~\ref{sec:metric_disagreement} reports that QLIKE and raw-variance
MSE rank our battery almost independently.
...
The paper reports one frozen battery of 98 arms on one panel of
$n = 218{,}934$ bars, with identical rows for every cell, organised into four
analyses that each vary one thing at a time:

\begin{enumerate}
    \item \textbf{Lag-ladder sweep} (Section~\ref{sec:powerlaw}). The estimator
      (OLS), the information set (backbone only) and the panel are held fixed
      while the geometric base and truncation cap of the lag ladder vary. This
      isolates quadrature error in the backbone itself.

    \item \textbf{Bucket attribution} (Section~\ref{sec:marginal}). The model
      class is held fixed at causally tuned ridge while the exogenous
      information set varies over the eight buckets and their union, so that
      information is never confounded with capacity.

    \item \textbf{Model-class comparison} (Section~\ref{sec:linvsnonlin}). The
      information set is held fixed while the estimator varies over five
      classes, all causally tuned, all refit every bar.

    \item \textbf{Algorithm ablations} (Section~\ref{sec:algorithms}). The
      feature set is frozen entirely and the estimation algorithm varies,
      including the matched embedding ablations of
      Table~\ref{tab:knn_ablation}.
\end{enumerate}

Superseded analyses --- the variable-$N$ subgroup tables and the
evaluation-panel hyperparameter search --- contribute no number to this paper;
Section~\ref{sec:limitations} explains why they were withdrawn.

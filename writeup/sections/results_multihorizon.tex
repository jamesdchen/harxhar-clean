...
This subsection asks two things of the exogenous block beyond that
horizon (Section~\ref{sec:multihorizon_methods}): whether its advantage
over the incumbent grows or decays as the target moves from the next
bar to the next week, refitting the paper's estimator directly at each
horizon; and, within the session, how far the one-step forecast reaches
into the variance remaining before the close.
...
\paragraph{The rest of the session.} The second object is the variance

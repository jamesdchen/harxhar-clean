...
The 90\% model confidence set \citep{HansenLundeNason2011} over the nine
feature sets is the singleton containing the joint bucket.
\pending{MCS elimination order and $p$-values; the claim is currently reported
without the table that supports it}

We attach an interpretive caution that the eventual table will not remove.
With $n = 218{,}934$ bar-level loss differentials, an MCS has enormous power,
and collapsing to a singleton is close to the default outcome for any
comparison in which one arm is even slightly and systematically ahead. The
singleton result should be read as ``the joint bucket is not statistically
tied with any singleton bucket'' and not as ``the joint bucket is the only
useful feature set''---a distinction that Table~\ref{tab:buckets}'s overlap
ratio already makes on economic grounds.
...
For the large effects this is immaterial---a $t$ of $-19$ survives any
correction one might apply to 98 comparisons. For the bottom of
Table~\ref{tab:buckets} it is not immaterial: \texttt{vol\_demand} at
$t = -4.1$, on an effect of $0.00078$ that carries $0.00082$ of benchmark
debt, is exactly the kind of finding that a family-wise correction plus a
corrected backbone would jointly dissolve. We do not claim it survives.
\pending{family-wise or FDR adjustment across the 98-arm battery}

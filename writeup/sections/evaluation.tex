...
% Contract section: comparator, loss, testing machinery, back-transform,
% inheritance. Frozen before any comparison; no results live here.

Four objects are fixed before any model is compared, and together they
constitute the evaluation framework every number in this paper inherits:
the comparator each arm is measured against and the testing machinery
that accompanies it (Section~\ref{sec:dp_benchmark}); the reason the
back-transform from the fitting scale to the variance scale carries an
explicit second moment (Section~\ref{sec:smearing_bias}); the calibrated,
strictly causal estimator of that second moment
(Section~\ref{sec:smearing_estimator}); and the inheritance rule that
makes the whole stack binding on every downstream comparison
(Section~\ref{sec:smearing_contract}).

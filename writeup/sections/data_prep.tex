...
grid contains $246{,}057$ rows, from 2005-01-02 to 2024-04-30. One
consequence of the calendar-only rule is disclosed here because it
produced a measurable incident (Section~\ref{sec:buckets}): bars after an
early close exist on the grid as forward-filled values, and on such bars
different feeds' availability indicators can disagree for a single bar.
\decide{root repair: derive dead-session bars from the data itself ---
drop grid bars on which the realized-variance source did not print, the
same observed-print fact the availability indicators already use ---
instead of the calendar-only rule. Threshold-free, but it changes the
panel row set and therefore every number in the paper; a frozen-panel
amendment for the author.}

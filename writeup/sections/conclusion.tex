...
rough-volatility correspondence implies. The sign structure fails: 8 of 12
kernel lags are negative, close to half of all exogenous cross-kernel cells
are, and the integrated kernel is $-0.038$---outside the $[0,1)$ range a linear
self-exciting process admits, with a sign that flips across sessions. We
therefore quote no branching ratio, and carry the kernel forward as an
instrument rather than as a structural model.
...
because their per-bar losses were not retained. (iii)~Our kernel estimate earns structural language for its shape but not for
its sign: the exponent is now estimated and agrees with theory, while the
negative mass and the inadmissible integrated kernel remain. Two design choices
are the leading suspects and are not separated by current evidence---a rolling
diurnal divisor that acts as a high-pass filter, and a probe that estimates the
self-kernel jointly with 41 exogenous channels, making it partial rather than
marginal. Removing both is the experiment that settles it. (iv)~A

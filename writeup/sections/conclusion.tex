...
heavy and uniform on the wide exogenous block. The recoverable
...
final model is linear in its parameters throughout: three blocks, three
fixed penalties, one exact rolling solve, and it outperforms the
...
more broadly. First, the evaluation contract is a modeling choice: the
...
preprocessing regime and one contract, and arms that survive only under
the legacy contract are labeled as such in the appendix. Several
protocol quantities remain marked pending in the text, and the
conditional-variance panel awaits re-scoring under the current contract.
The natural extensions are equally concrete: promoting the
mean-reverting second-moment refinement to the contract (a re-score of
every comparison); the frozen tree-expert bank, which puts the trees and
the product block on identical footing; and model confidence sets over

...
rough-volatility correspondence implies. The sign structure fails, on the archived
estimate: 8 of 12 kernel lags negative, close to half of all exogenous
cross-kernel cells negative, and an integrated kernel of $-0.038$, outside the
$[0,1)$ range a linear self-exciting process admits. Running the experiments
designed to explain that pathology exonerated both suspects and found something
worse---the archived kernel cannot be rebuilt from the committed raw data and
transform code, and a rebuild that matches the panel exactly shows none of the
pathology. We therefore quote no branching ratio, carry the kernel forward as
an instrument rather than a structural model, and treat its reproducibility as
the open problem.
...
because their per-bar losses were not retained. (iii)~The kernel estimate is not reproducible from source, which supersedes the
structural questions we had queued behind it. Both suspected causes of its sign
pathology---the rolling diurnal divisor and the partial-versus-marginal
distinction---were tested and exonerated; the rebuild is well behaved on every
diagnostic the archive fails. Persisting the feature cache and recording the
probe's conditioning set is the prerequisite for any structural work. Two
limitations were, however, settled along the way: the 5/95 target winsorization

...
significance. Refit directly at longer horizons, the exogenous
advantage grows with the horizon --- level at thirty minutes, and
largest at one week --- so the information in the wide block is about
slow-moving variance and pays most where the HAR lags say least; and
within the session the same per-bar forecast, using only what is known
at each bar, carries that advantage to the variance remaining before
the close.

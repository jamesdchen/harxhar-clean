...
$0.00976$ ($t = -11.3$). A fitted map with learned weights profits from many
...
reproduced---and beaten---by contracting each channel's rung ladder against a
fixed $u^{-1}$ weight and running plain ridge on the resulting 41 numbers.
Fifty-three columns reach QLIKE $0.13280$ against $0.13415$ for the 504-column
design with the matrix penalty ($t = -5.63$), $0.13571$ for the backbone alone
($t = -9.59$), and $0.13617$ for plain ridge on the 492 raw columns---worse
than the backbone it exists to improve. The effective-degrees-of-freedom
accounting shows the first two are the same estimator computed two ways: the
penalty spends $\approx 54$ degrees of freedom, which is $1 + 12 + 41$, the
dimension of its own null space. The stripped-down design is also an order of
...
\emph{What dominates all of it?} Protocol and data quality, by an order of
magnitude. A fixed training window rather than a rolling one is worth $0.11$
QLIKE. An imputation defect in our own pipeline---an availability indicator
computed after a forward fill, which silently disabled three guards this
codebase already contained for that exact pathology---put scaled features at
$2{,}260$ rolling interquartile ranges, cost $0.033$ in a single year, and
affects 23 of 41 channels. Selecting a shape parameter against the reported
loss rather than a validation tail is worth $0.0033$. Against these, the
model-class effects this literature argues about are $0.002$ to $0.004$. We
report our own defect at the same altitude as our results because the ordering
is the transferable finding: a team that gets the protocol and the data right
and fits a small fixed basis will beat a team that gets them wrong and fits
anything at all.

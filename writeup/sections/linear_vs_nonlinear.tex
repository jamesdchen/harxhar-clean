...
from $0.00184$ to $0.00358$. The systematic sign across all nine is the
strongest part of the evidence; the magnitude is small.

\paragraph{The pairwise significance is not established.} Every DM statistic in
this paper is computed against the shared incumbent, a third model. From
$t = -15.4$ for LightGBM and $t = -19.9$ for ridge on \texttt{all\_features},
one cannot infer that the $0.00184$ between them is distinguishable from zero:
the two loss series are strongly positively correlated, and the difference of
two significant improvements is not itself tested. This is the single largest
gap between claim and evidence in the paper, and it is closed by one run.
\pending{pairwise DM between matched tree and linear arms, and a model
confidence set over all 45 causally tuned arms; until these exist, the
model-class ordering is a consistent sign pattern across nine feature sets and
...
raw-variance OOS-$R^2$ is $+0.152$. Since lower QLIKE and higher $R^2$ both
denote better forecasts, agreement would produce a \emph{negative} correlation.
The observed value is near zero with the wrong sign: the two metrics are
essentially unrelated across this battery.
...
\begin{itemize}
  ...
  \item On the kernel dictionary of Section~\ref{sec:descriptive}, trees fail
    in both available seats: as a direct forecaster (an extrapolation clamp
    costing $+0.08$) and as a residual reader ($+0.0096$ against $-0.0016$ for
    a smooth local-linear reader on the same residual). The nonlinearity is
    real but smooth and diffuse, not tree-shaped.
\end{itemize}

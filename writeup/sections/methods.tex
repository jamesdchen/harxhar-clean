...
(Section~\ref{sec:attribution}); and the block ladder culminating in the
paper's final model,
...
\subsection{The Back-Transformation Bias}
...
\subsection{The Calibrated Back-Transformation}
...
bucket attribution of Sections~\ref{sec:buckets}--\ref{sec:joint_bucket},
the linear-class comparisons of Sections~\ref{sec:dense_weak} and
\ref{sec:dense_weak_ew}, and the tree and final-model results of
...
squared forecast before the baseline reversal. Convention \emph{calibrated}
...
rank-stability summary is the Kendall rank correlation between the calibrated-protocol
...
Kendall $\tau$ of each alternative ranking against the calibrated-protocol ranking
over all arms: none $\unifSmearTauNone$, traditional Duan
$\unifSmearTauDuan$.}
\label{tab:smear_sensitivity}
\begin{tabular}{@{}lccc@{}}
\toprule
 & No correction & Traditional Duan & Calibrated \\
\midrule
...
Section~\ref{sec:buckets_tuned}. All three penalized families are
...
boosting builds an additive ensemble
$F_M(x) = \sum_{m=1}^{M} \eta\, f_m(x)$ of shallow regression trees,
each $f_m$ fit to the negative gradient of the loss at the current
ensemble's predictions and scaled by a shrinkage rate $\eta$.
\textbf{XGBoost} \citep{Chen2016} fits each tree to a second-order
Taylor expansion of the loss: leaf weights are obtained in closed form
from the per-leaf gradient and Hessian sums with an $\ell_2$ penalty,
and the split criterion includes an explicit complexity penalty per
added leaf; row and column subsampling decorrelate successive trees.
\textbf{LightGBM} \citep{Ke2017} uses the same first-order boosting
scheme with engineering that matters at this design's width: features
are binned into histograms so split search costs are independent of the
window length, gradient-based one-side sampling concentrates the fit on
large-gradient rows, exclusive feature bundling compresses the many
mutually exclusive availability-indicator columns, and trees grow
leaf-wise under a size cap rather than level-wise. The two enter the
campaign in two distinct roles. In the causal-tuning battery they are
tuned arms like any other: their hyperparameters are re-selected on the
same periodic forward-split schedule as every tuned arm, and the
battery fully crosses the five estimators of this subsection with the
nine feature buckets on the battery's shared panel ($n = 218{,}934$
thirty-minute bars, identical rows in every cell). In the frozen
\emph{expert bank}, a menu of twenty fixed configurations --- the top
eight unique configurations plus two Sobol points per family from a
dev-span Optuna study, frozen permanently before any scored bar ---
is refit every bar on the identical wide design at the disclosed refit
cadence, so the tree class and the linear class are compared on the
same information set with no per-arm tuning freedom. The shared
comparator in both roles is the per-bar OLS--HAR incumbent

...
\textbf{Contract note.} The scoring contract's second-moment layer was
...
them --- are scored under the adaptive contract. Rows whose chunks
...
\emph{legacy} scalar-contract levels; the two contracts shift QLIKE
levels by roughly $5$--$8 \times 10^{-3}$, so legacy levels are not
comparable to adaptive-contract levels, while paired $\Delta$ and DM
within each contract remain valid. A mean-reverting GARCH$(1,1)$
...
would be a further change of contract.

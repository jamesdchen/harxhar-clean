...
the two measurements, the position rules, and the control; constants
were set before the book was scored.
...
inversion from the mid is applied.
...
\paragraph{Position rules.} Three rules are scored on the same $R_t$ and
...
\item \emph{Short volatility}: $q_t = -1$ every day. The rule takes no
      ...
      it, and the control for the other two.
\item \emph{Long--short volatility}: $q_t = \mathrm{sign}(s_t)$, long
      the package when the forecast exceeds implied variance and short
      otherwise. Direction only, at unit size.
\item \emph{Unit-median VRP}: signed and linear in the premium,
      \begin{equation}
      ...
      $|\mathrm{VRP}|$; the clip caps the position at three lots. This
      is the headline rule.
\end{itemize}
...
mostly that short. What the forecast can contribute at this hour is
size on the short, not direction.

...
For the first window, and for any window whose predecessor is missing or
empty, the map falls back to the identity
$(\hat{a}_k, \hat{b}_k) = (0, 1)$; every such window is flagged in the
harvest accounting.
...
value of window $k$'s forecasts can move it. For the first window, where
no predecessor exists, it falls back to the in-window scalar mean of
$(y^{\mathrm{raw}}_t - m_t)^2$; the fallback window is flagged in the
harvest accounting.
...
real-time forecast, not a retrospectively corrected one. The single
exception is the first window's second-moment fallback, which uses the
window's own residuals because no predecessor exists; it is flagged in the
harvest accounting.

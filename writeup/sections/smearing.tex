...
\citep{Duan1983}. Fix one evaluation window of $N$ bars. The inputs are
three per-bar arrays: the forecasts $\{\hat{y}_t\}_{t=1}^{N}$ on the
transformed (square-root, diurnally adjusted) scale, the diurnal baseline
factors $\{B_t\}_{t=1}^{N}$, where $B_t$ is the factor that reverses the
Stage-1 adjustment at bar $t$, and the \emph{unwinsorized} transformed
realizations $y_t = \sqrt{RV^{\mathrm{raw}}_t / B_t}$, reconstructed
exactly from the persisted raw target. The scorer computes a single scalar
smear from the window's own forecast residuals against that target,
\begin{equation}
...
\emph{Residuals against the unwinsorized target.} The residuals entering
Equation~\eqref{eq:smear_scalar} are computed against the unwinsorized
square-root-scale target, reconstructed exactly from the persisted raw
realization as $y^{\mathrm{raw}}_t = \sqrt{RV^{\mathrm{raw}}_t / B_t}$,
not against the winsorized fit target. With this choice the decomposition
of Equation~\eqref{eq:second_moment} holds for the evaluation target
itself: the second moment being added is the second moment of the errors
against the quantity actually scored, tails included. One bias remains and
is stated plainly: the \emph{coefficients} are estimated on the winsorized
fit target, so the forecast $\hat{y}_t$ itself carries the fit-scale
clipping, which is a modeling choice rather than a scoring defect. It is
shared by every arm and cancels to first order in comparisons.

...
\paragraph{How each product is computed.} A product column is not the raw
product of two panel columns; both construction steps are causal and both
matter. Each parent column is first standardized on a trailing window using
a \emph{floored} scale estimate --- the floor prevents a low-dispersion
window from inflating the standardized parent, the same failure mode the
scaling discipline of Section~\ref{sec:data_prep} guards against --- and the
product is then formed bar by bar from the two standardized parents, so
every product value at bar $t$ is a function of data available at $t$. The
selection statistic ranks candidates by the absolute correlation between
the product column and the backbone's out-of-sample residual on the frozen
selection window, so a product enters only for what it adds \emph{beyond}
the fitted backbone, not for restating level information the HAR block
already carries. \pending{the exact constants of the construction as frozen
in the campaign executor: trailing-window lengths for the parent scaling,
the floor rule, and the selection-window definition --- to be transcribed
from \texttt{src/unification.py} so the paper states precisely what ran}
...
$\Delta\text{QLIKE} = -0.00093$, DM $t = +3.56$. The mechanism is consistent

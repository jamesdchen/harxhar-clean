...
% the 4-block ridge: alpha=1 target-HAR + alpha=100 linear exog block +
% (arc reordered 2026-08-10: PCA factor block is now rung 3, products rung 4;
%  the blk3/blk4 macro names below still key the scorer's arm registry and
%  must be re-frozen under the new ladder at harvest)
...
four-block ridge, the final rung of the block ladder this paper builds
(two blocks in Section~\ref{sec:dense_weak}, three in
Section~\ref{sec:pca_factors}).
...
raw coefficients. After scaling, the four-block estimator runs in the same
...
The campaign executor runs the four-block arm under two conventions. The
...
($250$-day) window. Under the stated convention the four-block ridge
...
per-bar comparison of the four-block against the three-block ridge on the
...
\decide{whether the four-block table, once run, reports the tree arms
alongside it --- the natural closing comparison is the four-block ridge
...
penalties, refit every bar. The next section adds the fourth block.

...
features, and all $523$ exogenous columns. Table~\ref{tab:lasso_vs_ridge}
reports both estimators on that design, and, as a control, on the
no-exogenous design (HAR plus calendar only). Ridge scores QLIKE $0.12788$,
with a Diebold--Mariano statistic of $-19.9$ against the incumbent. The
lasso scores $0.12907$ at $-13.2$. Ridge's margin over the lasso is
$0.00119$. Keeping every column and shrinking produces a lower loss than
restricting the fit to the subset of columns the $\ell_1$ penalty selects.
On the control design the ordering inverts in sign against the incumbent:
both penalized arms lose to plain OLS, because shrinking a small,
well-conditioned design adds bias with no compensating variance reduction.

\begin{table}[H]
\centering
\caption{Ridge versus lasso, out-of-sample QLIKE with the Diebold--Mariano
$t$-statistic against the OLS--HAR incumbent in parentheses. $n = 218{,}934$
thirty-minute bars, identical rows in every cell; negative $t$ favors the
arm.}
...
Design & Ridge & Lasso \\
\midrule
Full ($523$ exogenous columns) & \textbf{0.12788} ($-19.9$) & 0.12907 ($-13.2$) \\
No-exogenous control           & \textbf{0.13445} ($+5.0$)  & 0.13475 ($+7.0$)  \\
\midrule
Incumbent (per-bar OLS--HAR)   & \multicolumn{2}{c}{\unifAzeroQLIKE{}} \\
\bottomrule
\end{tabular}
\end{table}
The DM statistic is the mean loss differential divided by its HAC standard
error, so ridge's larger $|t|$ also states that its per-bar edge over the
incumbent has lower serial-correlation-adjusted variance than the lasso's.
One caveat: both DM statistics are computed against the incumbent, a third
model, so the ridge-minus-lasso difference itself carries no significance
statement yet. \pending{pairwise ridge-vs-lasso DM (or the
incumbent-cancelling bound) on the full design, so the head-to-head margin
is tested rather than pointed at}

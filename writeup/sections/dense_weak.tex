...
features, and the full exogenous block --- under two penalty protocols:
fixed constants (ridge $\alpha = 1.0$, lasso $\alpha = 10^{-4}$) and causal
re-selection from the battery grids (Table~\ref{tab:lasso_vs_ridge}). The
two protocols order the estimators oppositely. Under causal tuning, ridge
($0.23480$, DM $-7.5$ against the incumbent) beats the lasso ($0.23571$,
$-6.8$). At the fixed pins, the lasso ($0.23297$, $-10.4$) beats ridge
($0.23442$, $-5.6$) and is the best linear arm in the campaign. Both tuned
arms lose to their fixed counterparts: causal re-selection costs more in
selection variance than it recovers, on both penalty families.
...
identical rows in every cell; negative $t$ favors the arm. Fixed penalties:
ridge $\alpha = 1.0$, lasso $\alpha = 10^{-4}$. Causally tuned: penalty
re-selected every $250$ solves from the battery grids on a forward
validation split.}
\label{tab:lasso_vs_ridge}
\begin{tabular}{@{}lcc@{}}
\toprule
Protocol & Ridge & Lasso \\
\midrule
Fixed penalty  & \unifBOneRidgeQLIKE{} (\unifBOneRidgeDM{}) & \unifBTwoLassoQLIKE{} (\unifBTwoLassoDM{}) \\
Causally tuned & \unifBOneRidgeTunedQLIKE{} (\unifBOneRidgeTunedDM{}) & \unifBTwoLassoTunedQLIKE{} (\unifBTwoLassoTunedDM{}) \\
\midrule
...
A penalty-level sweep resolves the apparent contradiction. Holding the
ridge family fixed and varying only the constant, QLIKE falls
monotonically in $\alpha$ across the tested grid: $0.23564$ at
$\alpha = 0.1$, $0.23511$ at $0.3$, $0.23442$ at $1$, $0.23384$ at $3$,
and $0.23343$ at $10$, still decreasing at the grid's edge. The penalty
\emph{level} moves the loss by more than the penalty \emph{family} does at
any fixed level, and the wide design rewards heavier shrinkage than
either arm's default pin --- the direction the block penalties of

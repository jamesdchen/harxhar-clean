...
re-selection from the battery grids (Table~\ref{tab:lasso_vs_ridge}). At
the fixed pins, the lasso (\unifBTwoLassoQLIKE{}, DM \unifBTwoLassoDM{}
against the incumbent) beats ridge (\unifBOneRidgeQLIKE{},
\unifBOneRidgeDM{}). Under causal tuning the ordering reverses: tuned
ridge (\unifBOneRidgeTunedQLIKE{}, \unifBOneRidgeTunedDM{}) beats tuned
lasso (\unifBTwoLassoTunedQLIKE{}, \unifBTwoLassoTunedDM{}). Tuning moves
the two families in opposite directions --- it improves ridge, whose fixed
pin of $\alpha = 1.0$ the sweep below shows to be far from the loss
minimum, and it costs the lasso, whose fixed pin already sits near its
optimum.
...
A penalty-level sweep explains the reversal. Holding the ridge family
fixed and varying only the constant, QLIKE falls monotonically in $\alpha$

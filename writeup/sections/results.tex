...
first-iteration scalar back-transform protocol, whose benchmark is QLIKE
\unifAzeroQLIKEScalar{} --- the incumbent every $\Delta$ and DM in this
table is computed against; under the current protocol the benchmark is
\unifAzeroQLIKE{} (Section~\ref{sec:smearing_contract}), and this grid's
re-harvest under it is in progress (Appendix~\ref{app:protocol_note}).
...
\midrule
(no exogenous $=$ incumbent) & \unifAzeroQLIKEScalar{} & --- & --- \\
\bottomrule
\end{tabular}
\end{table}
...
every cell, rows scored under the first-iteration scalar protocol
(benchmark \unifAzeroQLIKEScalar{}; current protocol
\unifAzeroQLIKE{}); re-harvest under the current protocol is in
progress. DM $t$ is against the benchmark; negative
...
parentheses. Panel and protocol as in Table~\ref{tab:buckets} (frozen
panel, $n = 273{,}554$ identical rows in every cell; first-iteration
scalar protocol, incumbent \unifAzeroQLIKEScalar{}); negative $t$ favors
the arm. Both arms re-select their penalty every $250$ solves from the
...
scored under the evaluation protocol. Sixteen of the seventeen
...
expert's harvest is completing at this writing; the partial coverage is
disclosed wherever it binds below.
...
selection arm scores $0.21848$ (DM $-3.3$ against the benchmark); the
hedged arm scores $0.21518$ (DM $-8.9$) --- \textbf{better than every
individual expert, including the best of them} ($0.21796$), a paired
improvement of $-0.0028$ over the strongest member. The ordering
(selection $<$ best expert $<$ hedge, with hedge $-0.0033$ better than
selection at DM $-4.5$) is the paper's recurring tuning lesson in its
...
rows above are $2003$+ by construction and, at this writing, cover
$139{,}438$ bars while one expert completes; the linear arms are scored
...
discipline of Section~\ref{sec:smearing_contract}), so the
trees-versus-products verdict is reserved for the scorer's paired
...
stated: the hedged bank's $-0.0100$ improvement over the benchmark at
DM $-8.9$, and the bank-internal ordering above.

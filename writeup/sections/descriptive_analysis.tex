...
($t = -15.8$, $p = 8.4 \times 10^{-19}$). Section~\ref{sec:rough} revises the
paper's position accordingly.
...
\paragraph{(i) The fitted kernel takes negative values, and not marginally.} A
linear Hawkes kernel must be non-negative; Equation~\eqref{eq:hawkes_intensity}
does not otherwise define a point process. Our fitted $\varphi$ violates this
badly. Pooled, \textbf{8 of 12 lags are negative}, carrying 10.3\% of total
absolute kernel mass. By session the violation is far worse: at the open,
87.8\% of the kernel's mass is negative, and after hours 86.3\%. The exogenous
cross-kernels are no better---between 44.7\% and 50.4\% of all
(channel, lag) cells are negative across the six sessions, which is
indistinguishable from a coin flip.

This is not a small blemish on an otherwise Hawkes-shaped object. Three
readings are available: a filter artifact (below), genuine inhibition requiring
the nonlinear Hawkes class \citep{BremaudMassoulie1996}, or the concession that
$\varphi$ is a regression weight function and not an intensity kernel at all.
On this evidence only the third is defensible, and it is the one we adopt.

\paragraph{(ii) The negative mass is what breaks the branching ratio.} The two
diagnostics are the same fact seen twice: an integrated kernel of $-0.038$ is
what a kernel with 10\% negative mass and a small positive part produces. The
sessions make the connection explicit---the two bins with the most negative
mass, after hours (86.3\%) and close (56.9\%), carry the two most negative
$\hat n$ values ($-0.260$ and $-0.130$).

\paragraph{(iii) The exponent is \emph{not} the anomaly we expected.} Earlier
drafts recorded the exponent as steeper than the literature's $\approx 1$ and
treated that as a second anomaly requiring explanation. Estimated rather than
assumed, $\hat\beta = 1.033$ with a 95\% interval of $[0.963, 1.104]$
(Equation~\eqref{eq:beta_estimate}) sits squarely on the literature's value.
There is no exponent anomaly to explain. What remains anomalous is the sign
structure, not the decay rate.

\paragraph{(iv) The sign structure is plausibly an artifact of Stage 1, and of
the probe design.} Our diurnal adjustment divides by a \emph{rolling}
20-observation same-slot mean (Equation~\eqref{eq:diurnal_mean}), a high-pass
filter rather than a fixed seasonal profile, and high-pass filtering induces
negative side lobes in an estimated kernel. Independently, the probe that
produced these coefficients regresses the target on its own ladder \emph{and}
41 exogenous channels jointly, so the self-kernel is a \emph{partial} effect
net of channels that are themselves functions of past volatility. A Hawkes
branching ratio is a marginal quantity; a partial one net of collinear
volatility proxies has no reason to be positive. Either mechanism alone would
generate the sign structure we observe.

\paragraph{The decisive experiment, now better specified.} Two changes must be
made together and their effects separated: re-estimate the kernel with Stage 1
removed, entering time-of-day as slot dummies rather than dividing by a rolling
slot mean; and re-estimate it \emph{marginally}, without the exogenous block,
so that the object is a self-excitation kernel rather than a partial effect. If
the negative mass collapses under either change, the sign structure is
manufactured and a structural reading becomes available; if it survives both,
the linear Hawkes correspondence should be abandoned in favour of the weaker
regression-kernel language. The exponent no longer needs this experiment---it
is estimated, and it agrees with theory.
\pending{marginal and unfiltered kernel re-estimation; until it is run we
describe the backbone as \emph{power-law-like}, quote no branching ratio, and
make no structural claim}
...
literature's $\approx 1$. The estimate reverses this. At
$\hat\beta = 1.033$ with a 95\% interval of $[0.963, 1.104]$, the
rough-volatility implied value of $1.6$ is rejected at
$t = -15.8$, while $1.0$ is not rejected at all. Under the
$H = \alpha - \tfrac{1}{2}$ mapping our estimate implies
$H \approx 0.03$, rougher than the $H \approx 0.1$ usually reported and close
to the boundary of the regime in which the limit theorem is stated.
...
the base-5 HAR ladder is a coarse quadrature. The kernel's \emph{shape} is
measured and behaves: the decay exponent is $1.033 \pm 0.036$, the power law
beats an exponential on both fit and forecast, and the exogenous operator is
rank 4--6. The kernel's \emph{sign structure} does not: 8 of 12 lags are
negative, the integrated kernel is $-0.038$, and both violate what a linear
self-exciting model requires. We therefore carry the power-law backbone forward
as an instrument and carry the Hawkes correspondence forward only as a source
of hypotheses. Three consequences follow:
...
\begin{enumerate}
  ...
  \item \textbf{Two later negative results are predictions, not surprises.}
    ...
    and the sign structure above is a standing obstacle to doing so.
\end{enumerate}

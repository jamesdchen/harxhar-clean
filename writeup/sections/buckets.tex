...
information: per-window loss decomposition attributes more than the
arm's entire net deficit to a single evaluation window, in which a small
number of bars carry forecast values far outside the arm's normal range
and the window-level correction spreads their residuals across every bar
of the window; excluding that one window the joint arm improves on the
incumbent. \decide{the single-window incident in the joint arm is under
investigation (same class as the market-aggregate window disclosed above:
localized numerical contamination amplified by a window-scalar
correction); the arm is to be re-run once the mechanism is closed, and no
conclusion about joint extraction under literal OLS should be drawn from
the current row} The same columns under the per-block shrinkage of

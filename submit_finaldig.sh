...
MASK=$(sbatch --parsable --export=ALL,CID=$CID sig_masks.sbatch)
...
for spec in "8 0.3 fa_d8c3" "10 0.3 fa_d10c3" "8 0.5 fa_d8c5" "8 0.1 fa_d8c1" "12 0.3 fa_d12c3"; do
  ...
  C=$(sbatch --parsable --dependency=afterany:$D --export=ALL,CID=$CID,ARM=resid_subset,LBL=$3 sig_collect.sbatch)
...
for spec in "2 0.05 200 50 fb_r2" "3 0.05 200 30 fb_r3" "4 0.05 200 20 fb_r4" "3 0.03 400 30 fb_r3n4" "6 0.05 150 10 fb_r6"; do
  set -- $spec
  D=$(sbatch --parsable --dependency=afterok:$MASK --export=ALL,CID=$CID,RDEPTH=$1,RLR=$2,RNEST=$3,RMCW=$4,LBL=$5 heroBxgb_dig.sbatch)
  C=$(sbatch --parsable --dependency=afterany:$D --export=ALL,CID=$CID,ARM=resid_regime,LBL=$5 sig_collect.sbatch)

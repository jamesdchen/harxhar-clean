...
sub () {  # depth rank wdv fmlin nknots label
  local J C
  J=$(qsub -terse -v CID=$CID,RDEPTH=$1,RANK=$2,WDV=$3,FMLIN=$4,BASIS=hinge,NKNOTS=$5,WD=0.1,RDIAG=false,EXPERT=fm,LBL=$6 moe_dig.sge | cut -d. -f1)
  C=$(qsub -terse -hold_jid "$J" -v CID=$CID,ARM=resid_regime,LBL=$6 sig_collect.sge)
